...
public:
  PlatformThreadDispatcher() {
    window_message_ = RegisterWindowMessage(L"MeloUnionJustAudioWindowsDispatch");
    window_class_name_ = L"MeloUnionJustAudioWindowsDispatchWindow";
  ...
  void Post(std::function<void()> task) {
    ...
    if (hwnd_ != nullptr && window_message_ != 0) {
      PostMessage(hwnd_, window_message_, 0, 0);
    }
  }
...
private:
  static LRESULT CALLBACK WndProc(
    ...
    if (message == WM_NCCREATE) {
      ...
      dispatcher = reinterpret_cast<PlatformThreadDispatcher*>(
        create_struct->lpCreateParams);
      SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(dispatcher));
    }
    if (dispatcher != nullptr && message == dispatcher->window_message_) {
      dispatcher->Drain();
      return 0;
    }
  ...
  HWND hwnd_ = nullptr;
  UINT window_message_ = 0;
  LPCWSTR window_class_name_ = nullptr;
  std::mutex mutex_;
...
public:
  ...
  JustAudioEventSink::JustAudioEventSink(
    ...
    state_ = std::make_shared<SinkState>();
    auto event_channel =
    ...
    auto event_handler = std::make_unique<flutter::StreamHandlerFunctions<>>(
      [state = state_](const EncodableValue* arguments, std::unique_ptr<flutter::EventSink<>>&& events) -> std::unique_ptr<flutter::StreamHandlerError<>> {
      state->sink = std::move(events);
      return nullptr;
    }, [state = state_](const EncodableValue* arguments) -> std::unique_ptr<flutter::StreamHandlerError<>> {
      state->sink.reset();
      return nullptr;
    });
  ...
  void Success(const EncodableValue& event) {
    ...
    auto state = state_;
    dispatcher->Post([state, event]() {
      if (!state->disposed && state->sink) {
        state->sink->Success(event);
      }
    });
  }
  ...
  void Error(const std::string& error_code,
    ...
    if (dispatcher == nullptr) return;
    auto state = state_;
    dispatcher->Post([state, error_code, error_message]() {
      if (!state->disposed && state->sink) {
        state->sink->Error(error_code, error_message);
      }
    });
  }
private:
...
public:
  AudioPlayer::AudioPlayer(
    ...
    event_sink_ = std::make_unique<JustAudioEventSink>(messenger, "com.ryanheise.just_audio.events." + idx, dispatcher);
    data_sink_ = std::make_unique<JustAudioEventSink>(messenger, "com.ryanheise.just_audio.data." + idx, dispatcher);
