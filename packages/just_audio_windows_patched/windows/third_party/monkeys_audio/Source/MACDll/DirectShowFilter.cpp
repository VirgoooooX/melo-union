...
#include "DirectShowSDK/wxutil.h"
#include <initguid.h>
#include <stdio.h>
...
HRESULT CAPEStream::OpenAPEFile(LPWSTR pFileName)
{
    ...
    m_rtDuration = static_cast<int64_t>(static_cast<double>(m_dDuration) * static_cast<double>(10000));
...
HRESULT CAPEStream::FillBuffer(IMediaSample * pSample)
{
    ...
    int64_t lSize = pSample->GetSize();

    int64_t iBlocksDecoded = 0;

    {
        ...
        rtStart = static_cast<int64_t>( static_cast<double>(m_iBlocksDecoded) * (static_cast<double>(10000000) / static_cast<double>(m_lSampleRate) ) );
        rtStart -= m_rtStart;
        rtStop = rtStart + 1;
