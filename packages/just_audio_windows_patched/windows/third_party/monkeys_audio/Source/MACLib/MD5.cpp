...
MD5Update (MD5_CTX *context, const void *in, unsigned int inputLen)
{
    ...
    if (inputLen >= partLen) {
        memcpy(static_cast<void *>(&context->buffer[index]), static_cast<const void *>(input),
            partLen);
    ...
    memcpy (static_cast<void *>(&context->buffer[index]), static_cast<const void *>(&input[i]),
        static_cast<size_t>(inputLen-i));
}
...
void MD5Final(unsigned char digest[16], MD5_CTX *context)
{
    ...
    memset (static_cast<void *>(context), 0, sizeof (*context));
}
