...
void CHyperLink::OnClicked()
{
    ...
    bool bResult = GotoURL(m_strURL, SW_SHOW);
    m_bVisited = (bResult != false);
    if (!m_bVisited)
    {
...
bool CHyperLink::GotoURL(LPCTSTR url, int showcmd)
{
    bool bResult = false;
    ...
    if (APE_POINTER_TO_INT64_WITH_NAMESPACE(result) <= HINSTANCE_ERROR) {
        ...
        if (GetRegKey(HKEY_CLASSES_ROOT, _T(".htm"), key) == ERROR_SUCCESS) {
            ...
            if (GetRegKey(HKEY_CLASSES_ROOT,key,key) == ERROR_SUCCESS) {
                ...
                USES_CONVERSION;
                if (WinExec(T2A(key), showcmd) > HINSTANCE_ERROR)
                    bResult = true;
            }
        }
    }
