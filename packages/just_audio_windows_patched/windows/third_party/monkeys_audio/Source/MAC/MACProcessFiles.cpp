...
CMACProcessFiles::~CMACProcessFiles()
{
    ...
    for (int z = 0; z < m_paryFiles->GetSize(); z++)
        m_paryFiles->ElementAt(z)->m_nKillFlag = APE_KILL_FLAG_STOP;
...
bool CMACProcessFiles::Pause(bool bPause)
{
    ...
    for (int z = 0; z < m_paryFiles->GetSize(); z++)
    {
        m_paryFiles->ElementAt(z)->m_nKillFlag = bPause ? APE_KILL_FLAG_PAUSE : APE_KILL_FLAG_CONTINUE;
    }

    // update the pause timer
    if (m_bPaused)
    {
        m_PausedDuration.Reset();
    }
    else
    {
        m_nPausedTotalMS += m_PausedDuration.GetElapsedMS();
    }
...
int CMACProcessFiles::GetPausedTotalMS() const
{
    ...
    if (m_bPaused)
    {
        nPausedMS += m_PausedDuration.GetElapsedMS();
    }
    return nPausedMS;
}
...
bool CMACProcessFiles::ProcessFile(int nIndex)
{
    ...
    m_paryFiles->ElementAt(nIndex)->m_ProcessingDuration.Reset();
...
void CMACProcessFiles::ProcessFileThread(MAC_FILE * pInfo)
{
        ...
        if (bMakesOutput && (nRetVal == ERROR_SUCCESS))
        {
            ...
            if ((nRetVal == ERROR_SUCCESS) && (pInfo->GetOutputExtension() == _T(".ape")) && (pInfo->m_strInputFilename.Right(3).CompareNoCase(_T(".wv")) == 0))
            {
                ...
                if (spAPETag->GetHasAPETag())
                {
                    ...
                    for (int z = 0; true; z++)
                    {
                        IAPETagField * pField = spAPETag->GetTagField(z);
                        if (pField == APE_NULL)
                            break;
            ...
            else if ((nRetVal == ERROR_SUCCESS) && (pInfo->GetOutputExtension() == _T(".ape")) && (pInfo->m_strInputFilename.Right(5).CompareNoCase(_T(".flac")) == 0))
            {
                ...
                if (spFLACTag->m_mapFields.GetCount() > 0)
                {
                    ...
                    if (spFLACTag->m_spPicture != APE_NULL)
                    {
                        ...
                        CSmartPtr<char> spExtensionANSI(CAPECharacterHelper::GetANSIFromUTFN(spFLACTag->m_strImageExtension), true);
            ...
            else if ((nRetVal == ERROR_SUCCESS) && (pInfo->GetOutputExtension() == _T(".ape")) && (pInfo->m_Tag.m_bFoundTag != false))
            {
                // copy tags from WAV, AIFF, etc to APE files
                CSmartPtr<APE::IAPETag> spAPETagNew(new CAPETag(pInfo->m_strOutputFilename, true));

                if (pInfo->m_Tag.m_spTitle != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_TITLE, pInfo->m_Tag.m_spTitle);
                if (pInfo->m_Tag.m_spArtist != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_ARTIST, pInfo->m_Tag.m_spArtist);
                if (pInfo->m_Tag.m_spAlbumArtist != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_ALBUM_ARTIST, pInfo->m_Tag.m_spAlbumArtist);
                if (pInfo->m_Tag.m_spAlbum != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_ALBUM, pInfo->m_Tag.m_spAlbum);
                if (pInfo->m_Tag.m_spYear != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_YEAR, pInfo->m_Tag.m_spYear);
                if (pInfo->m_Tag.m_spGenre != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_GENRE, pInfo->m_Tag.m_spGenre);
                if (pInfo->m_Tag.m_spComment != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_COMMENT, pInfo->m_Tag.m_spComment);
                if (pInfo->m_Tag.m_spComposer != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_COMPOSER, pInfo->m_Tag.m_spComposer);
                if (pInfo->m_Tag.m_spTrackNumber != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_TRACK, pInfo->m_Tag.m_spTrackNumber);
                if (pInfo->m_Tag.m_spDiscNumber != APE_NULL)
                    spAPETagNew->SetFieldString(APE_TAG_FIELD_DISC, pInfo->m_Tag.m_spDiscNumber);
                if (pInfo->m_Tag.m_spImage != APE_NULL)
                {
                    // get the MIME type
                    CString strMimeType; strMimeType.SetString(pInfo->m_Tag.m_spImageMimeType);

                    // trim if it's like image/JPEG
                    if (strMimeType.Find(_T("/")))
                        strMimeType = strMimeType.Right(strMimeType.GetLength() - strMimeType.Find(_T("/")) - 1);

                    // build the image tag
                    CSmartPtr<char> spExtensionANSI(CAPECharacterHelper::GetANSIFromUTFN(strMimeType), true);
                    int nExtensionLength = static_cast<int>(strlen(spExtensionANSI));
                    int nBufferBytes = static_cast<int>(pInfo->m_Tag.m_nImageBytes) + static_cast<int>(nExtensionLength) + 1;
                    CSmartPtr<char> spBuffer(new char [static_cast<size_t>(nBufferBytes)], true);
                    memcpy(spBuffer, spExtensionANSI, static_cast<size_t>(nExtensionLength)); // extension
                    spBuffer[nExtensionLength] = 0; // NULL after the extension
                    memcpy(&spBuffer[nExtensionLength + 1], pInfo->m_Tag.m_spImage.GetPtr(), static_cast<size_t>(pInfo->m_Tag.m_nImageBytes)); // image data
                    spAPETagNew->SetFieldBinary(APE_TAG_FIELD_COVER_ART_FRONT, spBuffer, static_cast<intn>(nBufferBytes), TAG_FIELD_FLAG_DATA_TYPE_BINARY);
                }
    ...
    pInfo->m_nProcessingElapsedMS = pInfo->m_ProcessingDuration.GetElapsedMS();
    pInfo->m_nRetVal = nRetVal;
    pInfo->m_bDone = true;
    pInfo->m_bNeedsUpdate = true;

    // update the status to be user cancelled if we stopped
    if ((pInfo->m_nRetVal == ERROR_UNDEFINED) && (pInfo->m_nKillFlag == APE_KILL_FLAG_STOP))
