...
import 'metadata_limits.dart';
import 'metadata_parser_exception.dart';
...
class Buffer {
  ...
  Uint8List read(int size) {
    _validateLength(size, operation: 'read');
    if (size > maxMetadataFrameBytes) {
      throw MetadataParserException(
        track: File(''),
        message: 'Declared read length $size exceeds the '
            '$maxMetadataFrameBytes-byte metadata frame limit',
      );
    }
    fileCursor += size;
  ...
  void skip(int length) {
    _validateLength(length, operation: 'skip');
    ...
    if (length <= remainingInBuffer) {
      ...
      fileCursor = currentPosition + length;
  ...
  void _validateLength(int length, {required String operation}) {
    final available = remainingBytes;
    if (length < 0 || length > available) {
      throw MetadataParserException(
        track: File(''),
        message: 'Cannot $operation $length bytes with only $available '
            'remaining bytes',
      );
    }
  }
