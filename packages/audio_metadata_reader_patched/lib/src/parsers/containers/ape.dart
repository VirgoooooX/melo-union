...
class ApeParser extends TagParser<ApeMetadata> {
  ...
  ApeMetadata parse(RandomAccessFile reader) {
    ...
    if (footer.version < 2000 ||
        footer.size < 32 ||
        footer.size > fileLength ||
        footer.size > maxMetadataBytes) {
      reader.closeSync();
  ...
  ApeMetadata? _tryParseItems({
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      final oversizedCover = itemType == 1 &&
          normalizedKey.startsWith('COVER_ART_') &&
          valueSize > maxEmbeddedArtworkBytes + 64 * 1024;
      if (valueSize > maxMetadataFrameBytes || oversizedCover) {
        _buffer.skip(valueSize);
        continue;
      }
  ...
  void _parseBinaryItem(
    ...
    if (imageBytes.length > maxEmbeddedArtworkBytes) {
      return;
    }
