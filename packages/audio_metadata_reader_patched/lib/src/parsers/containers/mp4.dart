...
class MP4Parser extends TagParser<Mp4Metadata> {
  ...
  /// [4...7] -> box name (ASCII)
  ///
  BoxHeader _readBox(Buffer buffer, {int? containerEnd}) {
    ...
    if (boxSize < 8 ||
        payloadSize > buffer.remainingBytes ||
        (containerEnd != null &&
            buffer.fileCursor + payloadSize > containerEnd)) {
      throw MetadataParserException(
        track: File(''),
        message: 'Malformed MP4 box length $boxSize exceeds remaining bytes',
      );
    }
  ...
  void processBox(Buffer buffer, BoxHeader box) {
    if (box.type == "moov") {
      ...
      if (payloadSize > maxMetadataFrameBytes) {
      ...
      final payloadSize = box.size - 8;
      if (payloadSize > maxMetadataFrameBytes ||
          _metadataBytes + payloadSize > maxMetadataBytes) {
        buffer.skip(payloadSize);
        return;
      }
      _metadataBytes += payloadSize;
      ...
      switch (boxName) {
        ...
        case "ART":
          ...
          if (normalized != null) {
            if (!tags.artists.contains(normalized)) {
              tags.artists.add(normalized);
            }
            tags.artist ??= normalized;
          }
          break;
        case "aART":
          final normalized = _normalizedCredit(value);
          if (normalized != null) {
            if (!tags.albumArtists.contains(normalized)) {
              tags.albumArtists.add(normalized);
            }
            tags.albumArtist ??= normalized;
          }
          break;
        case "alb":
          tags.album = value;
        ...
        case "covr":
          ...
          if (imageData.length <= maxEmbeddedArtworkBytes) {
      ...
      final boxEnd = buffer.fileCursor + box.size - 8;
      final mean = _readBox(buffer, containerEnd: boxEnd);
      String.fromCharCodes(buffer.read(mean.size - 8)); // mean value

      final name = _readBox(buffer, containerEnd: boxEnd);

      final nameValue =
          String.fromCharCodes(buffer.read(name.size - 8).sublist(4));
      final dataBox = _readBox(buffer, containerEnd: boxEnd);
  ...
  String? _normalizedCredit(String value) {
  ...
  void parseRecurvise(Buffer buffer, BoxHeader box) {
    ...
    final containerEnd = buffer.fileCursor + limit;
    int offset = 0;
    ...
    while (offset < limit) {
      final newBox = _readBox(buffer, containerEnd: containerEnd);
