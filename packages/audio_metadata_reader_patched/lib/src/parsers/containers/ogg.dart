...
class OGGParser extends TagParser<VorbisMetadata> {
  ...
  VorbisMetadata _parseVorbisComment(
    ...
    for (int i = 0; i < userCommentListLength; i++) {
      ...
      offset += 4;
      if (commentLength > maxMetadataFrameBytes ||
          _metadataBytes + commentLength > maxMetadataBytes) {
        throw MetadataParserException(
          track: File(''),
          message: 'Vorbis comments exceed safe metadata limits',
        );
      }
      _metadataBytes += commentLength;

      while (dataBuffer.length - offset < commentLength) {
        final nextPage = _parseUniquePage().data;
        if (dataBuffer.length + nextPage.length > maxMetadataBytes) {
          throw MetadataParserException(
            track: File(''),
            message: 'Vorbis comment pages exceed the '
                '$maxMetadataBytes-byte metadata limit',
          );
        }
        dataBuffer.addAll(nextPage);
      }
