...
/// Decode every text value in a frame.
///
/// ID3v2.4 permits several values in one text frame separated by NUL. The
/// historical single-value helper above intentionally keeps its compatibility
/// behavior, while artist frames use this helper to preserve every credit.
List<String> getTextValuesFromFrame(Uint8List information) {
  if (information.isEmpty) return const [];
  final decoded = switch (information.first) {
    0 => latin1Decoder.convert(information, 1),
    1 => utf16Decoder.decodeUtf16Le(information.sublist(1)),
    2 => utf16Decoder.decodeUtf16Be(information.sublist(1)),
    3 => utf8Decoder.convert(information, 1),
    _ => getTextFromFrame(information),
  };
  return decoded
      .split('\u0000')
...
class ID3v2Parser extends TagParser<Mp3Metadata> {
  ...
  Mp3Metadata parse(RandomAccessFile reader) {
    ...
    final tagEnd = 10 + min(size, min(buffer.remainingBytes, maxMetadataBytes));
    ...
    while (buffer.fileCursor + 10 <= tagEnd) {
      ...
      final remainingTagBytes = tagEnd - buffer.fileCursor;
      if (frame.size > remainingTagBytes) {
        if (frame.id == 'APIC') {
          return metadata;
        }
        throw MetadataParserException(
          track: File(''),
          message: 'Declared ID3 frame length ${frame.size} exceeds '
              'remaining tag bytes $remainingTagBytes',
        );
      }
  ...
  void processFrame(String frameId, int size) {
    ...
    final handlers = switch (frameId) {
      "APIC" => () {
          if (fetchImage) {
            ...
            try {
              ...
              if (picture.bytes.length <= maxEmbeddedArtworkBytes) {
                metadata.pictures.add(picture);
              }
            } on RangeError {
              // A damaged picture frame must not discard safe text metadata.
            } on FormatException {
              // A damaged picture frame must not discard safe text metadata.
            }
          } else {
      ...
      "TPE1" => () {
          final content = buffer.read(size);
          final values = getTextValuesFromFrame(content);
          metadata.leadPerformers.addAll(values);
          metadata.leadPerformer = values.firstOrNull;
        },
      "TPE2" => () {
          final content = buffer.read(size);
          final values = getTextValuesFromFrame(content);
          metadata.bandsOrOrchestras.addAll(values);
          metadata.bandOrOrchestra = values.firstOrNull;
        },
      "TPE3" => () {
          final content = buffer.read(size);
