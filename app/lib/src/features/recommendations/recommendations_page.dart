...
class _RecommendationsPageState extends ConsumerState<RecommendationsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final providers = repository.providerEntries
        .where((entry) => entry.isEnabled && entry.provider.isAuthenticated && entry.descriptor.supports(ProviderCapability.readDailyRecommendations))
    ...
    final tracks = source?.allTracks().where((track) => track.isPlayable).toList(growable: false) ?? const [];
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              FilledButton.icon(
                onPressed: tracks.isEmpty ? null : () => repository.playTrack(tracks.first),
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        ...
                        subtitle: Text('${track.artists.join(' / ')} · ${track.album ?? '今日推荐'}'),
                        trailing: Row(
