  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return FutureBuilder<List<ProviderSearchResults>>(
      ...
      builder: (context, snapshot) {
        ...
        final count = filtered.fold<int>(0, (sum, group) => sum + group.tracks.length);
        if (count == 0) {
          return Center(
            child: Text(
              '没有找到“$query”相关的结果。',
...
class _SearchTrackRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final repository = ref.watch(demoRepositoryProvider);
    final availability = repository.favoriteWriteAvailability(track.ref.providerId);
    return ListTile(
      ...
      subtitle: Text('${track.artists.join(' / ')} · ${track.album ?? providerName}'),
      trailing: Row(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: Theme.of(context).textTheme.bodySmall?.copyWith(
              color: netease ? MeloColors.neteaseForeground : MeloColors.qqForeground,
              fontWeight: FontWeight.w700,
            ),
      ),
    );
