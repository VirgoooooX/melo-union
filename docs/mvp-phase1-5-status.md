...
- 网易云 Provider：`packages/provider_netease` 已接入真实网易云 Web 搜索端点，并以 experimental Provider 注册进 app；设置页可导入/清除本机 Cookie 会话，启动时会注入 Provider 以读取账号资料、喜欢列表、用户歌单和每日推荐。写收藏、播放票据、下载票据和音质 level 解析已有代码路径与 mock 测试覆盖；本机真实 Cookie 已 smoke 到无损 FLAC 票据，但写收藏仍需官方客户端抽检。
...
- 真实 Provider：网易云 experimental Provider 已接入真实搜索，并支持通过设置页导入本机 Cookie 读取账号资料/喜欢列表/用户歌单/每日推荐；已将 Android 端与 Windows 端会话升级至系统级加密存储，并完成真实账号播放/下载音质 smoke。尚未完成写收藏官方端验证。QQ Provider 仍未接入。
- 真实播放资源：网易云 Provider 已能生成播放/下载票据；fake Provider 返回 `provider://...` URI，仅用于验证票据流和桥接，不能代表实际音频播放。
...
1. Windows 优先闭环：用 Windows debug app 验证搜索、喜欢列表、用户歌单、每日推荐、音质切换和播放条状态。
2. 网易云 Phase 2 闭环：读取喜欢后进行写收藏/取消收藏官方端验证。
3. 用网易云当前账号允许的真实 HTTP media URL 在 Windows 桌面和 Android 真机上验证播放稳定性；Android 再补通知栏、音频焦点和蓝牙控制。
4. 将生产启动目录接到 `music_data_drift` 仓储。
5. 基于已可构建的 Windows runner 实现 SMTC native bridge。
